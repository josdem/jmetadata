...
public class MainWindow extends JFrame {
  ...
  /** Default button style private static final String SEND_BUTTON_NAME = "button[A-Za-z0-9]+"; */
  ...
  @Autowired private LoginWindow loginWindow;
  @Autowired private ViewEngineConfigurator viewEngineConfigurator;
  @Autowired private ControlEngineConfigurator controlEngineConfigurator;
            ...
            new Observer<ObservValue<ImagePanel>>() {
              ...
              public void observe(ObservValue<ImagePanel> t) {
                ...
                List<Metadata> metadatas =
                    controlEngineConfigurator.getControlEngine().get(Model.METADATA);
  ...
  void onLoadMetadata(List<Metadata> metadataList) {
    ...
    for (Metadata metadata : metadataList) {
      ...
      descriptionTable.setValueAt(
          metadata.getTrackNumber(), row, ApplicationConstants.TRACK_NUMBER_COLUMN);
      descriptionTable.setValueAt(
          metadata.getTotalTracks(), row, ApplicationConstants.TOTAL_TRACKS_NUMBER_COLUMN);
      descriptionTable.setValueAt(
          metadata.getCdNumber(), row, ApplicationConstants.CD_NUMBER_COLUMN);
      descriptionTable.setValueAt(
          metadata.getTotalCds(), row, ApplicationConstants.TOTAL_CDS_NUMBER_COLUMN);
      if (metadata.isMetadataFromFile()) {
        descriptionTable.setValueAt(
            ApplicationConstants.NEW, row, ApplicationConstants.STATUS_COLUMN);
        metadataWithAlbum.add(metadata);
        getApplyButton().setEnabled(true);
      } else {
        descriptionTable.setValueAt(
            ApplicationConstants.READY, row, ApplicationConstants.STATUS_COLUMN);
      }
    }
  }
  ...
  void onReadyToApplyMetadata(MetadataAlbumValues metadataValues) {
    ...
    for (int i = 0; i < model.getRowCount(); i++) {
      ...
      if (!StringUtils.isEmpty(tracks)) {
        getDescriptionTable()
            .getModel()
            .setValueAt(tracks, i, ApplicationConstants.TOTAL_TRACKS_NUMBER_COLUMN);
      }
  ...
  private JMenuItem getLastFmMenuItem() {
    if (lastFmMenuItem == null) {
      ...
      lastFmMenuItem.addActionListener(
          new ActionListener() {
  ...
  public JButton getCompleteMetadataButton() {
    if (completeMetadataButton == null) {
      ...
      completeMetadataButton.addActionListener(
          new ActionListener() {
            ...
            private void resetStatus() {
              ...
              for (int i = 0; i < getDescriptionTable().getModel().getRowCount(); i++) {
                getDescriptionTable()
                    .getModel()
                    .setValueAt(ApplicationConstants.READY, i, ApplicationConstants.STATUS_COLUMN);
              }
            }
          });
    }
  ...
  public JTable getDescriptionTable() {
    if (descriptionTable == null) {
      ...
      descriptionTable.addMouseListener(
          new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
              if (e.getButton() == MouseEvent.BUTTON3) {
                MetadataDialog metadataDialog =
                    new MetadataDialog(
                        MainWindow.this, controlEngineConfigurator, "Alter ALL rows");
              ...
              new TableModelListener() {
                ...
                public void tableChanged(TableModelEvent e) {
                  if (e.getType() == TableModelEvent.UPDATE
                      && tableLoaded
                      && e.getColumn() != ApplicationConstants.STATUS_COLUMN) {
                    ...
                    getDescriptionTable()
                        .getModel()
                        .setValueAt(
                            ActionResult.NEW.getValue(), row, ApplicationConstants.STATUS_COLUMN);
                    ...
                    .request(
                        ...
                        new ResponseCallback<ActionResult>() {
                          ...
                          public void onResponse(ActionResult result) {
                            log.info(
                                "Writing metadata to {} w/result: {}", metadata.getTitle(), result);
                          ...
                          public void onResponse(ActionResult response) {
                            ...
                            if (response.equals(ActionResult.NEW)) {
                              ...
                              getDescriptionTable()
                                  .getModel()
                                  .setValueAt(
                                      metadata.getAlbum(), i, ApplicationConstants.ALBUM_COLUMN);
                              getDescriptionTable()
                                  .getModel()
                                  .setValueAt(
                                      metadata.getTrackNumber(),
                                      i,
                                      ApplicationConstants.TRACK_NUMBER_COLUMN);
                              getDescriptionTable()
                                  .getModel()
                                  .setValueAt(
                                      metadata.getTotalTracks(),
                                      i,
                                      ApplicationConstants.TOTAL_TRACKS_NUMBER_COLUMN);
                              getDescriptionTable()
                                  .getModel()
                                  .setValueAt(
                                      metadata.getCdNumber(),
                                      i,
                                      ApplicationConstants.CD_NUMBER_COLUMN);
                              getDescriptionTable()
                                  .getModel()
                                  .setValueAt(
                                      metadata.getTotalCds(),
                                      i,
                                      ApplicationConstants.TOTAL_CDS_NUMBER_COLUMN);
                            }
                            getDescriptionTable()
                                .getModel()
                                .setValueAt(
                                          ...
                                          if (response.equals(ActionResult.NEW)) {
                                            ...
                                            getDescriptionTable()
                                                ...
                                                .setValueAt(
                                                    metadata.getArtist(),
                                                    i,
                                                    ApplicationConstants.ARTIST_COLUMN);
                                            getDescriptionTable()
                                                .getModel()
                                                .setValueAt(
                                                    metadata.getTitle(),
                                                    i,
                                                    ApplicationConstants.TITLE_COLUMN);
                                            getDescriptionTable()
                                                .getModel()
                                                .setValueAt(
                                                    metadata.getAlbum(),
                                                    i,
                                                    ApplicationConstants.ALBUM_COLUMN);
                            ...
                            MainWindow.this
                                ...
                                .request(
                                    ...
                                    new ResponseCallback<ActionResult>() {
                                      ...
                                      public void onResponse(ActionResult response) {
                                        log.info(
                                            "response in completeValuesForAlbum for "
                                                + metadata.getTitle()
                                                + ": "
                                                + response);
                                        if (response.equals(ActionResult.NEW)) {
                                          ...
                                          if (response.equals(ActionResult.NEW)) {
                                            ...
                                            getDescriptionTable()
                                                ...
                                                .setValueAt(
                                                    metadata.getYear(),
                                                    i,
                                                    ApplicationConstants.YEAR_COLUMN);
                                            getDescriptionTable()
                                                .getModel()
                                                .setValueAt(
                                                    metadata.getGenre(),
                                                    i,
                                                    ApplicationConstants.GENRE_COLUMN);
                                            getDescriptionTable()
                                                .getModel()
                                                .setValueAt(
                                                    ActionResult.NEW.getValue(),
  ...
  private void afterComplete(Set<Metadata> metadataWithOutArtist) {
    if (!metadataWithOutArtist.isEmpty()) {
      controlEngineConfigurator
          .getControlEngine()
          .set(Model.METADATA_ARTIST, metadataWithOutArtist, null);
      getApplyButton().setEnabled(true);
  ...
  private class DescriptionTableModelListener implements TableModelListener {
    ...
    public void tableChanged(TableModelEvent e) {
      if (MainWindow.this.getCompleteMetadataButton().getText().equals(ApplicationConstants.APPLY)
          && e.getColumn() != ApplicationConstants.STATUS_COLUMN) {
